...
protocol NetworkServiceProtocol {
    ...
    func request(name: String, completion: @escaping (Result<[City], NetworkError>) -> Void)
}
...
extension NetworkService: NetworkServiceProtocol {
    ...
    func request(name: String, completion: @escaping (Result<[City], NetworkError>) -> Void) {
        ...
        let stringURL = "https://geocode-maps.yandex.ru/1.x?apikey=1c2c0a14-73c0-4a9f-a447-0bf7be0dfb59&format=json&geocode=" + name
        
        if let stringURL = stringURL.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed), let url = URL(string: stringURL) {
            let task = URLSession.shared.dataTask(with: url, completionHandler: { [weak self] data, response, error in
                ...
                let citys = self?.decodeAllWithSerializator(data)
    ...
    func decodeAllWithSerializator(_ data: Data) -> [City] {
        
        var citys = [City]()
        
        do {
            guard let dictionary = try JSONSerialization.jsonObject(with: data, options: []) as? Dictionary<String, Any> else { return citys }
            guard let response = dictionary["response"] as? [String: Any] else { return citys }
            guard let geoObjectCollection = response["GeoObjectCollection"] as? [String: Any] else { return citys }
            guard let featureMember = geoObjectCollection["featureMember"] as? [[String: Any]] else { return citys }
            
            featureMember.forEach { featureMember in
                
                guard let geoObject = featureMember["GeoObject"] as? [String: Any] else { return }
